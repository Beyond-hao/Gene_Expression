...
def load_model(
  project:str, model_name:str, device:Union[str, int]='cpu', host:str=DEFAULT_WANDB_HOST, alias:str='latest', checkpoint_file:str='model.ckpt'
    ...
    # Borzoi
    if project.lower() == "borzoi".lower():
        if model_name.lower() == "mouse_fold0".lower():
            checkpoint_file = "~/.cache/wandb/artifacts/obj/md5/56/1a699890e48890c152d8d0e7ea059f"
        elif model_name.lower() == "human_fold0".lower():
            checkpoint_file = "~/.cache/wandb/artifacts/obj/md5/16/dfc7d5b97a746d666ada7b8f583a40"
        else:
            print("project in [borzoi, enformer], model_name in [mouse_fold0, human_fold0]")

    model = LightningModel.load_from_checkpoint(checkpoint_file, map_location=device)
    return model
