...
class BaseModel(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Returns:
            Output tensor
        """
        x = self.embedding(x)  # shape: 4 * 768 * 1536
...
class DNA_Conv1D_Model(nn.Module):
    def __init__(self, expression_type):
        super(DNA_Conv1D_Model, self).__init__()

        self.expression_type = expression_type
        self.conv1 = nn.Conv1d(in_channels=4, out_channels=64, kernel_size=15, stride=1, padding=7)  # padding="same"
        self.bn1 = nn.BatchNorm1d(64)
        self.pool1 = nn.MaxPool1d(kernel_size=5, stride=5)
        self.dropout1 = nn.Dropout(0.3)

        self.conv2 = nn.Conv1d(in_channels=64, out_channels=128, kernel_size=15, stride=1, padding=7)
        ...
        self.conv4 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=15, stride=1, padding=7)
        self.bn4 = nn.BatchNorm1d(32)
        self.pool4 = nn.MaxPool1d(kernel_size=5, stride=5)
        self.dropout4 = nn.Dropout(0.3)

        self.fc_input_dim = 32 * (196608 // 625)

        self.fc1 = nn.Linear(self.fc_input_dim, 1024)
        self.fc2 = nn.Linear(1024, 78)  

    def forward(self, x):
        # Conv1D -> Activate -> Normalize -> pooling -> Dropout
        ...
        if self.expression_type == "minmax":
            x = F.relu(self.fc2(x))
        else:
            x = F.relu(self.fc2(x))
