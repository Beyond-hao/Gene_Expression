    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
            Args:
                x : Input data.
        """
        # print(f"--> ConvHead, before channel_transform, the shape: {x.shape}") # [4, 768, 1536]
        x = self.channel_transform(x)

        # print(f"--> ConvHead, after channel_transform, the shape: {x.shape}")  # [4, 86, 1536]
        x = self.pool(x)
        
        # print(f"--> ConvHead, after pool, the shape: {x.shape}")               # [4, 86, 1]
        # x = torch.sigmoid(x)
        return x
